...
if( isset( $_POST['token'] ) && !empty( $_POST['token'] ) && isset( $_POST['get'] ) &&  $_POST['get']=="true" )
{
    if( isset( $_SESSION['GET_PARTNER_DATA_FULL'] ) && !empty( $_SESSION['GET_PARTNER_DATA_FULL'] ) && $_POST['token']==$_SESSION['GET_PARTNER_DATA_FULL'] )
    {
        ...
        $query_partners=mysqli_query( $con,"SELECT pd.id,pd.first_name,pd.last_name,pd.main_all_acc FROM form_information_submitted pd INNER JOIN partner_auth pa ON pa.info=pd.id WHERE `status`=2 AND `email_verify`=0 AND `mobile_verify`=0 ORDER BY `id` DESC" );
        if( $query_partners )
        {
