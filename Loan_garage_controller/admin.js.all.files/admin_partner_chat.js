...
function set_msg(){
    ...
    if(validate_msg(msg)){
        $.ajax({
            url:"./set_admin_partner_chat.php",
            type:"POST",
            data:{"get":true,"msg":msg,"partner_id":partner_id},
...
function __user_chat_get(type_fetch){
    $.ajax({
        url:"./get_partner_admin_chat.php",
        type:"POST",
        data:{"get":true,"type":type_fetch,"partner_id":partner_id},
        success:function(data){
            data=$.parseJSON(data);
            if( Object.entries(data).length>0 ){
...
function set_status(){
    $.ajax({
        url:"./set_msg_status.php",
        type:"POST",
        data:{"set":true,"partner_id":partner_id},
        success:function(data){
            data=$.parseJSON(data);
            if( Object.entries(data).length>0 ){
                if(data.constructor==Object){
                    if(data.status=="success"){
                        ...
                        window.location.href="./index.php?data="+data.data;
                    }
                }else{
