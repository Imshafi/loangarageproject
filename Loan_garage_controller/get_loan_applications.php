<?php
    ...
    if(isset($_POST['get']) && $_POST['get']==="true" && isset( $_POST['type_get'] ) && is_numeric( $_POST['type_get'] ) && ( $_POST['type_get']==0 || $_POST['type_get']==2 || $_POST['type_get']==1 )  ){
        ...
        if(isset($_POST['from']) && isset($_POST['to'])){
            ...
            $from=$from." 00:00:00";
            $to=$to." 23:59:59";
            $sql="SELECT `id`,`f_name`,`status_loan`,`samaym_req`,`response`,`mukhyam` FROM `loan_apply_form` WHERE `mobile_confrim`=? AND `email_confrim`=? AND samaym_req >=? AND samaym_req <=? AND `status_loan`=?";
