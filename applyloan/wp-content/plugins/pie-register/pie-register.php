		...
		*/
		...
		function print_multi_lang_script_vars(){
			...
			$this->pie_pr_dec_vars_array = array(
				...
				'ValidationMsgText55'					=> __("* Invalid File Type","pie-register"),
...
?>
				...
				if(sizeof($unverified )==1)
				{
					...
					else
					{
						if ( $this->InvCodeExpired($user_id) )
						{
							...
							<p>
							<strong>'.ucwords(__("error","pie-register")).'</strong>: '.apply_filters("piereg_invalid_verification_key",__("The account cannot be activated. Please contact site administrator","pie-register"))).'
							</p>
							  </div>';
						} else {
		...
		function saveFields()
		{
			...
			if(isset($_POST['piereg_reg_form_nonce']) && wp_verify_nonce( $_POST['piereg_reg_form_nonce'], 'piereg_wp_reg_form_nonce' ))
			{
				...
				{
					$this->pie_post_array['error_message'] = __("Form title is required.","pie-register");
				} else {
					...
					if ( $user_id ) 
					{
						...
						if($register_type == "admin_email_verify")
						{
							if ( !$this->InvCodeExpired($user_id) )
							{
								...
								$this->pie_post_array['error'] = __("Invitation code has expired", "pie-register");
							}
						}else{
							if ( !$this->InvCodeExpired($user_id) )
							{
					...
					foreach ($users as $user_key=>$user_value){
						...
						else if ( $verification_status == 0 )
						{
							switch($register_type){
								case "email_verify":
									$verification_status = __("E-mail Verification","pie-register");
								break;
								case "admin_verify":
									$verification_status = __("Admin Verification","pie-register");
								break;
								case "admin_email_verify":
									$verification_status = __("E-mail & Admin Verification","pie-register");
								break;
								case "payment_verify":
									$verification_status = __("Payment Verification","pie-register");
								break;
							}
						} else {	
