				...
				if(sizeof($unverified )==1)
				{
					...
					if($user_login == $_GET['pie_id'])
					{
						...
						if ( !$obj_PieRegister->InvCodeExpired($user_id) ) 
						{
							...
							$login_error = '<strong>'.ucwords(__("error","pie-register")).'</strong>: '.apply_filters("piereg_invitation_code_expired",__("The account cannot be activated. Please contact site administrator","pie-register"));
						}
					}
					else
					{
